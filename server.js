...
// Allow both JSON and raw text for /api/tap
app.use("/api/tap", express.json());
app.use("/api/tap", express.text({ type: "text/*" }));
...
app.use((req, res, next) => {
  console.log(
    `[REQUEST] ${req.method} ${req.path} - Content-Type: ${
...
app.get("/api/debug/uid/:uid", async (req, res) => {
  try {
    ...
    const todayLogs = await Log.find({ uid, date: today }).sort({
      timestamp: -1,
    });
    const allUserLogs = await Log.find({ uid })
      .sort({ timestamp: -1 })
      .limit(10);
...
const MONGO =
  process.env.MONGODB_URI || "mongodb://localhost:27017/rfid_attendance";
...
mongoose
  ...
  .then(() => {
    ...
    server.listen(PORT, () => {
      ...
      console.log(
        `[SERVER] Debug logs at http://localhost:${PORT}/api/debug/logs`
      );
