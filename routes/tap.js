...
router.post('/', async (req, res) => {
  try {
    ...
    // If request body is a raw buffer/string, attempt to parse
    if (Buffer.isBuffer(req.body)) {
      ...
      try {
        ...
        console.log('[TAP-ROUTER] JSON parse failed, attempting repair:', error.message);

        // Try repairing malformed JSON
        let fixedBody = bodyStr
          .replace(/"uid"([A-Fa-f0-9]+)"/g, '"uid":"$1"')  // {"uid"123ABC"} → {"uid":"123ABC"}
          .replace(/"uid"([A-Fa-f0-9]+)}/g, '"uid":"$1"}') // {"uid"123ABC} → {"uid":"123ABC"}
          .replace(/"uid"([A-Fa-f0-9]+)/g, '"uid":"$1"');  // {"uid"123ABC → {"uid":"123ABC"}

        try {
          parsedBody = JSON.parse(fixedBody);
          console.log('[TAP-ROUTER] Successfully repaired JSON:', parsedBody);
        } catch (repairError) {
          console.log('[TAP-ROUTER] Repair failed, fallback extraction:', repairError.message);

          // Try regex extraction for UID
    ...
    if (!uid) {
      ...
      return res.status(400).json({ error: true, message: 'UID is required', code: 'MISSING_UID' });
    }

    // Normalize UID
    const cleanUid = uid.toString().trim().toUpperCase().replace(/[^0-9A-F]/g, '');
    console.log('[TAP-ROUTER] Cleaned UID:', cleanUid);

    if (!cleanUid || cleanUid.length < 6) {
      return res.status(400).json({ error: true, message: 'Invalid UID format', code: 'INVALID_UID' });
    }
    ...
    // Get today’s logs
    const todayLogs = await Log.find({ uid: cleanUid, date: today }).sort({ timestamp: -1 });

    // Daily tap limit
    if (todayLogs.length >= 8) {
      return res.json({ error: false, message: 'Daily tap limit reached (8 taps).', name: user.name, code: 'LIMIT_REACHED' });
    }

    // Determine tap type (alternate in/out)
    let tapType = todayLogs.length === 0 || todayLogs[0].type === 'tap-out' ? 'tap-in' : 'tap-out';
    ...
    await newLog.save();
    console.log(`[TAP-ROUTER] SUCCESS: ${tapType} recorded for ${user.name} (UID: ${cleanUid})`);

    // Get recent logs for response
