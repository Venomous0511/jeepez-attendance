...
function appendLog(log) {
  ...
  const logDate =
    log.date || new Date(logTimestamp).toLocaleDateString("en-US");
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    if (uid.length !== 7 || !/^[A-F0-9]{7}$/.test(uid)) {
      alert(
        `UID must be exactly 7 hexadecimal characters (0-9, A-F). You entered: "${uid}"`
      );
      return;
    }
    ...
    if (!/^09\d{9}$/.test(phoneNumber)) {
      alert(
        `Phone number must start with 09 and be 11 digits. You entered: "${phoneNumber}"`
      );
      return;
    }
...
document
  .getElementById("searchRegistered")
